...
void LoadDrawPalette()
{
    ...
    hpalFind = FindResource(NULL, sRsrc, "PALETTE");
    ...
    for (pI = 0; pI < 256; pI++)
    {
        ...
        {
            colorRefMap[pI] = RGB(c->rgbRed, c->rgbGreen, c->rgbBlue);
        }
    }
}
...
//	mdc=new CDC;
    ...
    if (InDrawRange())
    {
        ...
        dc->StretchBlt(0, 0, w, h, &mdc, 0, 0, imgs[i]->w, imgs[i]->h, SRCCOPY);
#else

        dc->StretchBlt(0, 0, w, h, &mdc, 0, 0, w, h, SRCCOPY);
