...
void ReportControl::SelectItem(int i)
{
    ...
    SetSelectionMark(i);
    SetItemState(i, LVIS_SELECTED | LVIS_FOCUSED, 0xFF);
    RedrawItems(i, i);
    UpdateWindow();
}

int ReportControl::TextSearch(int i, char * s, bool wrap, bool caseIgnore)
{
    CString ss = s;
    return (TextSearch(i, ss, wrap, caseIgnore));
}

int ReportControl::TextSearch(int i, CString ss, bool wrap, bool caseIgnore)
{
    // int j=0;
    int     ic, jc, ii, jj;
    int     fi;
