...
typedef struct OBJ_INDEX_MAP
{
    union {
        struct
        {
            short rides[SV6_OBJDAT_NUMENTRIES_RIDES], smallScenery[SV6_OBJDAT_NUMENTRIES_SMALLSCENERY],
        ...
        short items[SV6_OBJDAT_NUMENTRIES_ALL];
    };
...
typedef struct OBJ_USED_FLAGS
{
    union {
        struct
        {
            bool rides[SV6_OBJDAT_NUMENTRIES_RIDES], smallScenery[SV6_OBJDAT_NUMENTRIES_SMALLSCENERY],
                largeScenery[SV6_OBJDAT_NUMENTRIES_LARGESCENERY], walls[SV6_OBJDAT_NUMENTRIES_WALLS],
                banners[SV6_OBJDAT_NUMENTRIES_BANNERS], paths[SV6_OBJDAT_NUMENTRIES_PATHS],
                pathUtilities[SV6_OBJDAT_NUMENTRIES_PATHUTILITIES], groups[SV6_OBJDAT_NUMENTRIES_GROUPS],
                entrance[SV6_OBJDAT_NUMENTRIES_ENTRANCE], palette[SV6_OBJDAT_NUMENTRIES_PALETTE],
        ...
        bool items[SV6_OBJDAT_NUMENTRIES_ALL];
    };
...
CParkDatDlg::CParkDatDlg(CWnd * pParent /*=NULL*/) : CDialog(CParkDatDlg::IDD, pParent)
{
...
BOOL CParkDatDlg::OnInitDialog()
{
    ...
    {
        MessageBox("RCT2 Installation not found\r\n", "Warning", MB_OK);
...
DAT_TABLE_INDEXES::DAT_TABLE_INDEXES()
{
    ...
    static struct
    {
        WORD numEntries;
        char text[4];
    } ti[DAT_TABLE_ENTRY__NUM_ENTRIES] = { { SV6_OBJDAT_NUMENTRIES_RIDES, SV6_OBJDAT_ABBV_RIDES },
...
GetDatSlotTypStr(SUPER_STRING & text, int i)
{
    int                      j;
    bool                     inRange;
    static DAT_TABLE_INDEXES table;

    for_i0(j, DAT_TABLE_ENTRY__NUM_ENTRIES)
    {

        inRange = (i >= table.tableIndexes[j].lo) && (i <= table.tableIndexes[j].hi);
        if (inRange)
        {
...
SetLVItem(LVITEM lvi, UINT mask, int iItem, int iSubItem, UINT state, UINT stateMask, LPTSTR pszText, int cchTextMax,
...
CParkDatDlg::InitDatListCtrl()
{
    ...
    for_i0(i, SV6_OBJDAT_NUMENTRIES_ALL)
    {
        s.Number(3, 3, i);
#if 1
        datList.InsertItem(LVIF_TEXT | LVIF_PARAM, i, (char *)~s, 0, 0, 0, i);
#endif
    }
    ...
    selMark = datList.GetSelectionMark();
}

CParkDatDlg::LoadSettings()
{
    FILE * f;
...
void CParkDatDlg::OnButtonSv6Name()
{
    ...
    GetSaveFileName_Quick(sz(appData.sv6Name), ~appData.sv6Name, OFN_EXPLORER | OFN_FILEMUSTEXIST,
                          "RCT2 Park File (SV6)\0*.sv6", "sv6", "Select SV6 Park File");
...
void CParkDatDlg::OnButtonAddDat()
{
    ...
    if (GetSaveFileName_Quick(sz(addDatName), ~addDatName, OFN_EXPLORER | OFN_FILEMUSTEXIST,
                              "RCT2 Object Data File (DAT)\0*.dat", "dat", "Select RCT2 DAT File"))
    {
...
int FindFreeSlot(short objType)
{
    ...
    &dt = sv6.fileDataA.availableItems, &datTable = dt;
...
{
    int li = 0, nItems = datList.GetItemCount(), d;
...
CParkDatDlg::UpdateViewTableItem(int i)
{
    ...
    char * listText[3] = { isEmpty_Str, curDatName, menuName };
    ...
    else
    {
        ...
        memset(&cdat.stringTable[0].entries, 0, sz(cdat.stringTable[0].entries));
...
void CParkDatDlg::OnButtonReplaceSelectedDat()
{
        ...
        ;
    {
                ...
                )
        {
            ...
            {
                if ((i >= RCT2_BASES[cdat.objType]) && (i <= RCT2_BASES[cdat.objType + 1]))
                {
                    if (i < RCT2_BASES[OT__NUM_TYPES])
                    {
                        ReplaceItem(i, ~addDatName);
...
CParkDatDlg::SetMenusToAvailable()
{
    ...
    if (sv6Loaded)
    {
#define setAll(a) memset(&a, sd, sz(a))
...
CParkDatDlg::OutputMsg(int code, int d, char * s1, char * s2)
{
    OutputMsg(code, d, s1, d, s2);
}

CParkDatDlg::OutputMsg(int code, int d1, char * s1)
{
...
CParkDatDlg::OutputMsg(int code, int d1, char * s1, int d2, char * s2)
{
    DWORD   l;
    ...
    case OPC__SV6_OPENED:
        oa.Format(IDS__FORMAT__SV6_OPENED, s1);
        break;
    case OPC__SV6_SAVED:
        oa.Format(IDS__FORMAT__SV6_SAVED, s1);
        break;
    case OPC__SV6_OPENING:
        oa.Format(IDS__FORMAT__SV6_OPENING, s1);
        break;
    case OPC__SV6_SAVING:
        oa.Format(IDS__FORMAT__SV6_SAVING, s1);
        break;
    case OPC__ODAT_OPENED:
        break;
    case OPC__ODAT_SAVED:
        break;
    case OPC__ODAT_REPLACED:
        oa.Format(IDS__FORMAT__DAT_REPLACED, d1, s1, s2);

        break;
    case OPC__ODAT_DELETED:
        oa.Format(IDS__FORMAT__DAT_DELETED, d1, s1);

        break;
    case OPC__ODAT_ADDED:
        break;
    case OPC__ODAT_HEADERS_UPDATED:
        oa.Format(IDS___FORMAT___DAT__HEADERS_UPDATED);
        break;
    }
...
void GetUsedObjects(OBJ_USED_FLAGS & usedObjs)
{
    ...
    {
        usedObjs.banners[i] = ((DWORD)sv6.fileDataA.availableItems.banners[i].flags != (DWORD)0xFFFFFFFF);
    }
    for_i0(i, SV6_OBJDAT_NUMENTRIES_GROUPS)
    {
        usedObjs.groups[i] = ((DWORD)sv6.fileDataA.availableItems.groups[i].flags != (DWORD)0xFFFFFFFF);
    }
