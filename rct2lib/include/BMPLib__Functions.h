...
{
    return (false || (rect1.left != rect2.left) || (rect1.top != rect2.top) || (rect1.right != rect2.right) ||
...
{
    return (false || (a.blue != b.blue) || (a.green != b.green) || (a.red != b.red));
}
...
bool operator==(RGBQUAD a, RGBQUAD b)
{
    ...
    cmp = memcmp(&a, &b, sz(RGBQUAD) - 1);

    return (cmp == 0);
}

typedef FLAG_8 IMG16_4PLANE;
...
typedef BYTE PAL_COLORMAP[COLORMAP_MAXCOLORLEVELS][COLORMAP_MAXCOLORLEVELS][COLORMAP_MAXCOLORLEVELS];
...
typedef struct PAL_COLORMAP
{
    ...
    AddColorToTable(int palI, int r, int g, int b);
...
typedef struct BITMAP_FILE
{
    ...
    void * ImgMalloc(bool useHeader, int bitsPerPlane, int numPlanes, int width, int height);
    ...
    {
        UpdateHeaderPointers();
        return ((infoHeader->biPlanes == planes) && (infoHeader->biBitCount == bits));
    }
    InitPalColorMap(RGBQUAD * palette, WORD numEntries, bool mallocFlag, bool ignorePureBlack);
    int LoadJascPal(char * palFile, bool ignorePureBlack);
    int LoadMSPal(char * palFile, bool ignorePureBlack);
    int LoadPal(char * palFile, bool ignorePureBlack);
    ...
    SetDimensions_ColorDepth(int bitsPerPixelPlane, int numPlanes)
    {
        ...
        if (colorUsed > 256)
            colorUsed                           = 0;
        (header->header16M.bmiHeader).biClrUsed = (header->header16M.bmiHeader).biClrImportant = colorUsed;
    }
    SetDimensions(int width, int height, int bitsPerPixelPlane, int numPlanes)
    {
    ...
    {
        int sizePlanePixel_inBits, numPlanes, height, sizeImage, sizePlane, sizePlaneRow, topDown, bitOffset;
    ...
    RECT GetImgRect()
    {
        ...
        RECT area = { 0, 0, __max(infoHeader->biWidth, 1) - 1, __max(infoHeader->biHeight, 1) - 1 };
        return area;
    }
    ...
    {
        bool result = (x >= r.left) && (x <= r.right) && (y >= r.top) && (y <= r.bottom);
        return result;
    }
    Flip();
    Mirror();
    ...
    CopyHeaderFromBmp(BITMAP_FILE bmpToCopy, bool mallocFlag);
    // operators
