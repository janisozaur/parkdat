...
int GetSaveFileName_Quick(int filename_size, char * filename, DWORD flags, char * filters, char * defExt, char * title);

int ReadOpen(const char * filenamef);

int WriteOpen(const char * filename, bool doOverwritePrompt);
...
int QuickWrite(void * buffer, const char * filename, int fileOffset, DWORD count, bool doOverwritePrompt);
...
typedef struct FILENAME
{
    ...
    int __cdecl Scan(const char * format, ...);
...
int GetSaveFileName_SuperQuick(FILENAME & filename, char * defExt);
