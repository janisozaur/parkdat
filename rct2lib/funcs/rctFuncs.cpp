...
#define RCTREGKEYNAME "HKEY_LOCAL_MACHINE\\SOFTWARE\\FISH Technology Group\\RollerCoaster Tycoon Setup"
...
int GetRctPath(char * path)
{
    ...
    HKEY   hkey;
    DWORD
    size = HIGHFULLFILE, datatype = REG_SZ;

    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, RCTREGKEYNAME, 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
    {
...
int BrowseRctFile(int accessType, char * retFilename, int fileType)
{
    int          error;
    OPENFILENAME ofn;
    char         filetitle[HIGHFTL] = "", filename[HIGHFULLFILE] = "", filepath[HIGHFULLFILE] = "", title[64], *filter, *defExt;
    ...
    if (GetSaveFileName_Quick(HIGHFULLFILE, filename, flags, filter, defExt, title))
    {
