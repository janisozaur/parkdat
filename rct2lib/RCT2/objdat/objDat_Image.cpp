...
dllPort OBJDAT_FILE::AddImage(int imgI, short xofs, short yofs, IMG_256 & img, short imgEncdType, short seeThruColor)
{
    WORD width, height;
    DWORD
    alignWidth, tmpImgSz, newScanLineSize;
    WORD x, y, i, j;
    WORD curSpan;
#define TMP_IMG__MAX_SZ 1000000
        ...
        for_i0(y, img.h)
        {
            ...
            while (x < img.w)
            {
                ...
                if ((x < img.w) && (img.pix(x, y) != seeThruColor))
                {
                    ...
                    while ((x < img.w) && (img.pix(x, y) != seeThruColor))
                    {
                        sc->imgData[sc->numBytes] = img.pix(x, y);
                        ++sc->numBytes;
                        ++x;
                    }
    ...
    case IMG_IF_IMAGEDATA:
        ...
        {
            ++numImages;
            imageDirectory = (DATGRAPHINFOSTRUCT *)realloc(imageDirectory, sz(*imageDirectory) * numImages);
            if (!append)
            {
                memmove(&imageDirectory[imgI + 1], &imageDirectory[imgI], sz(*imageDirectory) * (numImages - (imgI + 1)));
            }

#define idir imageDirectory[imgI]
#define idirNext imageDirectory[imgI + 1]
            if (!append)
                idir.startaddress = idirNext.startaddress;
            else
            {
                idir.startaddress = scanLineSize - objImgSz;
            }
            idir.xofs   = xofs;
            idir.yofs   = yofs;
            idir.width  = img.w;
...
#undef idirNext
            if (!append)
            {
                ...
                memmove(&imageBufr[imageDirectory[imgI + 1].startaddress], &imageBufr[imageDirectory[imgI].startaddress],
                        scanLineSize - imageDirectory[imgI + 1].startaddress);
            }
            memcpy(&imageBufr[imageDirectory[imgI].startaddress], tmpImgBufr, objImgSz);
        }
        else
        {
...
dllPort DWORD OBJDAT_FILE::GetImageBufferSize()
{
    union {
        DWORD imgBufrSz, ibSz, ibs;
    };
    int i;
    union {
        DWORD maxStartAddrI, mxSaI;
    };
    short                x;
    ...
    if (numImages > 0)
    {
        ...
        for_i0(i, numImages)
        {
            if (imageDirectory[i].startaddress > imageDirectory[mxSaI].startaddress)
                mxSaI = i;
        }
        i  = mxSaI;
        id = &imageDirectory[i];
        ...
        case IMG_IF_IMAGEDATA_COMPACT:
            ...
            if (id->height > 0)
            {
                cRow = &imageBufr[ct[id->height - 1] + id->startaddress];
                done = false;
                while (!done)
                {
                    done = sc->rowEnd;
