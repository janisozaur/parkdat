...
typedef struct
{
    ...
    DWORD
    flags;
    BYTE TrackType[3], minCarsPerTrain, maxCarsPerTrain,
...
typedef struct
{
    ...
    DWORD
    fill02, flags; // zero
    BYTE  height, cursorsel;
    short buildfee, removefee;
    DWORD
    ...
    BYTE BaseIndex, fill1B; // zero in DAT files
} TYPE1HEADER;

typedef struct
{
    WORD reserved0; // Always zero in files
    DWORD
    reserved1;              // Always zero in files
    char  CursorSel, flags; // bit 3 = 3D sign
    WORD  BuildFee;
    short RemoveFee;
    DWORD
    reserved2;
    BYTE reserved3, // Always zero in files
...
typedef struct
{
    short Row,      // div by 32  (0xffff means end of list)
        Col,        // div by 32
        Z;          // div by 16
    BYTE clearance, // shr by 2
        unknown1,   //
...
typedef struct
{
    ...
    BYTE  cursorsel, Flags, clearance, effects; // first nibble = visibility (0 = opaque),upper nibble: 1 = animated (8 frames)
    WORD  buildFee;                             // *10.
    BYTE  reserved0C,                           // Always zero in files
        scrolling;                              // 0xFF if not scrolling
} TYPE3HEADER;

typedef struct
{                   // Banners : length = 12.
    WORD reserved0; // always zero
    DWORD
    reserved1;      // always zero
    BYTE Scrolling, // 0xFF if not scrolling, otherwise,
                    // indicates scrolling direction, location
        Flags;
    WORD BuildFee, // X 10. refund is 75% (rounded down)
...
{                   // Path Additions : length = 14.
    ...
    reserved1; // Always zero in files
    WORD Flags;
    BYTE Subtype, CursorSel;
    WORD BuildFee;  // X 10.
    BYTE reserved2, // Always zero in files
...
typedef union {
    union {
        TYPE0HEADER typ0, typRide;
    };
    union {
        TYPE1HEADER typ1, typSScen;
    };
    union {
        TYPE2HEADER typ2, typLScen;
    };
    union {
        TYPE3HEADER typ3, typWalls;
    };
    union {
        TYPE4HEADER typ4, typBanners;
    };
    union {
        TYPE5HEADER typ5, typPaths;
    };
    union {
        TYPE6HEADER typ6, typPathAdd;
    };
    union {
        TYPE7HEADER typ7, typGroup;
    };
    union {
        TYPE8HEADER typ8, typEntrance;
    };
    union {
        TYPE9HEADER typ9, typWater;
    };
    union {
        TYPE10HEADER typ10, typSText;
    };
} TYPESPECHEADER;
...
#define IMG_IF_IMAGEDATA_COMPACT (IMG_IF_IMAGEDATA | IMG_IF_COMPACT)

typedef struct
{
    DWORD startaddress;
    short width, height, xofs, yofs; // signed
    WORD  flags, unused;             // to pad the structure to 16 bytes
} DATGRAPHINFOSTRUCT;
...
const short RCT2_RIDES__BASE_TABLE_INDEX         = 0,
            RCT2_SMALLSCENERY__BASE_TABLE_INDEX  = RCT2_RIDES__BASE_TABLE_INDEX + RCT2_OBJMAX_RIDES,
            RCT2_LARGESCENERY__BASE_TABLE_INDEX  = RCT2_SMALLSCENERY__BASE_TABLE_INDEX + RCT2_OBJMAX_SMALLSCENERY,
            RCT2_WALLS__BASE_TABLE_INDEX         = RCT2_LARGESCENERY__BASE_TABLE_INDEX + RCT2_OBJMAX_LARGESCENERY,
            RCT2_BANNERS__BASE_TABLE_INDEX       = RCT2_WALLS__BASE_TABLE_INDEX + RCT2_OBJMAX_WALLS,
            RCT2_PATHS__BASE_TABLE_INDEX         = RCT2_BANNERS__BASE_TABLE_INDEX + RCT2_OBJMAX_BANNERS,
            RCT2_PATHADDITIONS__BASE_TABLE_INDEX = RCT2_PATHS__BASE_TABLE_INDEX + RCT2_OBJMAX_PATHS,
            RCT2_THEMEGROUPS__BASE_TABLE_INDEX   = RCT2_PATHADDITIONS__BASE_TABLE_INDEX + RCT2_OBJMAX_PATHADDITIONS,
            RCT2_PARKENTRANCES__BASE_TABLE_INDEX = RCT2_THEMEGROUPS__BASE_TABLE_INDEX + RCT2_OBJMAX_THEMEGROUPS,
            RCT2_WATER__BASE_TABLE_INDEX         = RCT2_PARKENTRANCES__BASE_TABLE_INDEX + RCT2_OBJMAX_PARKENTRANCES,
            RCT2_SCENARIOTEXT__BASE_TABLE_INDEX  = RCT2_WATER__BASE_TABLE_INDEX + RCT2_OBJMAX_WATER,
            RCT2__TABLE_END___BASE_TABLE_INDEX   = RCT2_SCENARIOTEXT__BASE_TABLE_INDEX + RCT2_OBJMAX_SCENARIOTEXT,
            RCT2_BASES[]                         = { RCT2_RIDES__BASE_TABLE_INDEX,         RCT2_SMALLSCENERY__BASE_TABLE_INDEX,
                             RCT2_LARGESCENERY__BASE_TABLE_INDEX,  RCT2_WALLS__BASE_TABLE_INDEX,
                             RCT2_BANNERS__BASE_TABLE_INDEX,       RCT2_PATHS__BASE_TABLE_INDEX,
                             RCT2_PATHADDITIONS__BASE_TABLE_INDEX, RCT2_THEMEGROUPS__BASE_TABLE_INDEX,
                             RCT2_PARKENTRANCES__BASE_TABLE_INDEX, RCT2_WATER__BASE_TABLE_INDEX,
                             RCT2_SCENARIOTEXT__BASE_TABLE_INDEX,  RCT2__TABLE_END___BASE_TABLE_INDEX };

#define RCT2__OBJDAT__STRING_TABLE__ENTRY__MAX_LENGTH 0x100

#define RCT2__OBJDAT__STRING_TABLE__ENTRY__NUM_LANGUAGES 14
...
typedef struct
{
    ...
    {
        BYTE lang;
        char text[RCT2__OBJDAT__STRING_TABLE__ENTRY__MAX_LENGTH];
...
#define LARGE_SCENERY__MAX_POS_STRUCTS (LARGE_SCENERY__MAX_SIZE * LARGE_SCENERY__MAX_SIZE)
...
typedef struct
{
    union {
        ...
        struct
        {
            short y, x,           //*32
                baseHeight;       //*16
            BYTE clearanceHeight; //*8
            union {
                struct
                {
                    BYTE unk0 : 1, unk1 : 1, unk2 : 1, unk3 : 1, unk4 : 1, noSupports : 1, unk6 : 1, unk7 : 1;
                };
                BYTE unkFlags;
            };
            union {
                struct
                {
                    BYTE wall1 : 1, wall2 : 1, wall3 : 1, wall4 : 1, quad1 : 1, quad2 : 1, quad3 : 1, quad4 : 1;
                };
                BYTE wallQuad;
            };
        };
...
dllPort typedef struct OBJDAT_FILE
{
    ...
    dllPort void  DecodeObjDatFile(), DecodeObj_Optional(BYTE *decdChunk, DWORD &i),
        ...
        EncodeObj_Optional_Ride(BYTE *decdChunk, DWORD &i), EncodeObj_Optional_SmScenery(BYTE *decdChunk, DWORD &i),
        EncodeObj_Optional_LgScenery(BYTE *decdChunk, DWORD &i), EncodeObj_Optional_Group(BYTE *decdChunk, DWORD &i),
