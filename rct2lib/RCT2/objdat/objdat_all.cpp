...
dllPort OBJDAT_FILE::OBJDAT_FILE()
{
...
dllPort bool OBJDAT_FILE::HasGroupInfo()
{
    switch (objType)
    {
...
dllPort WORD OBJDAT_FILE::GetNumStringTables()
{
    switch (objType)
    {
    case OT_RIDESHOP:
...
#define r optional.ride
    ...
    {
        memmove(&r.colorSchemes[j], &decdChunk[i], sz(r.colorSchemes[j]));
    ...
    for_i0(j, 4)
    {
        ...
        if (curLength > 0)
        {
            curLength = __min(sz(r.vehicleLoad[j].loadingPos), curLength);

            memmove(r.vehicleLoad[j].loadingPos, &decdChunk[i], curLength);
...
dllPort DWORD OBJDAT_FILE::Images_GetBufrSz()
{
...
#define id imageDirectory[maxStartAddrI]
        ...
        default:
            bufSz = id.startaddress + (id.width * id.height);
            break;
        }
...
dllPort OBJDAT_FILE::DecodeObjDatFile()
{
    ...
    retBufSz = DecodeSv6Chunk(&decdChunk_header, decdChunk, BUFSZ_MAX, &objEncd.chunkHdr, objEncd.chunk, 0);
...
#define movObjHeader(a)                                                                                                        \
    memmove(&this->objHeader.a, &decdChunk[i], sz(objHeader.a));                                                               \
...
#undef movObjHeader
    ...
    BYTE stringI = 0, strTableI = 0, numStrTables = GetNumStringTables();

    while (strTableI < numStrTables)
    {
        strTableDone = false;
        stringI      = 0;
        while (stringI < RCT2__OBJDAT__STRING_TABLE__ENTRY__NUM_LANGUAGES)
        {
            stringTable[strTableI].entries[stringI].lang = decdChunk[i++];
            strTableDone                                 = stringTable[strTableI].entries[stringI].lang == BYTE(0xff);
            if (!strTableDone)
            {
                strncpy(stringTable[strTableI].entries[stringI].text, (char *)&decdChunk[i],
    ...
    if (HasGroupInfo())
    {
        memmove(&groupInfo, &decdChunk[i], sz(groupInfo));
        i += sz(groupInfo);
    }
...
#define r optional.ride
    ...
    for_i0(j, r.numColorSchemes)
    {
        memmove(&decdChunk[i], &r.colorSchemes[j], sz(r.colorSchemes[j]));
        i += sz(r.colorSchemes[j]);
    }
    for_i0(j, 4)
    {
        ...
        if (curLength > 0)
        {
            ...
            memmove(&decdChunk[i], r.vehicleLoad[j].loadingPos, curLength);
            i += curLength;
        }
    }
...
#define DOASM 1
    ...
    sum_copy = sum;
    sum_Bytes[0] ^= data;
    sum = ((sum_copy << 11) & 0xfffff800) | ((sum_copy >> (32 - 11)) & 0x7ff);
#endif
}
...
#define DoCalc() CalcObjChecksum_XorRotate(sum, hdrBytes[i++])
...
#undef DoCalc
#define DoCalc() CalcObjChecksum_XorRotate(sum, decdChunk[i++])
...
dllPort OBJDAT_FILE::EncodeObjDatFile()
{
    ...
    case OT_RIDESHOP:
#define movObjHeader(a)                                                                                                        \
    memmove(&decdChunk[i], &this->objHeader.a, decdChunk_curSpan = sz(objHeader.a));                                           \
    i += decdChunk_curSpan
...
#undef movObjHeader
    ...
    while (strTableI < numStrTables)
    {
        while (stringI < RCT2__OBJDAT__STRING_TABLE__ENTRY__NUM_LANGUAGES)
        {
            ...
            {
                strncpy((char *)&decdChunk[i], stringTable[strTableI].entries[stringI].text,
    ...
    if (HasGroupInfo())
    {
        memmove(&decdChunk[i], &groupInfo, sz(groupInfo));
        i += sz(groupInfo);
    }
    ...
    retBufSz = EncodeSv6Chunk(ET_RLE, CHUNK_DATA__MAX_SIZE, &objEncd.chunkHdr, objEncd.chunk,
                              decdChunk_header.chunkSize_Encoded, &decdChunk_header, &decdChunk[i], 0);
...
dllPort OBJDAT_FILE::LoadObjDatFile(char * datName)
{
    ...
    if (datFile != NULL)
    {
        ...
        fread(&objEncd.objFileHeader, sz(objEncd.objFileHeader), 1, datFile);
        fread(&objEncd.chunkHdr, sz(objEncd.chunkHdr), 1, datFile);

        objEncd.chunk = (BYTE *)malloc(objEncd.chunkHdr.chunkSize_Encoded);
        fread(objEncd.chunk, objEncd.chunkHdr.chunkSize_Encoded, 1, datFile);
...
dllPort OBJDAT_FILE::SaveObjDatFile(char * datName, bool freeChunkBuff)
{
    ...
    if (datFile != NULL)
    {
        ...
        fseek(datFile, 0, SEEK_SET);
        fwrite(&objEncd.objFileHeader, sz(objEncd.objFileHeader), 1, datFile);
        fwrite(&objEncd.chunkHdr, sz(objEncd.chunkHdr), 1, datFile);

        fwrite(objEncd.chunk, objEncd.chunkHdr.chunkSize_Encoded, 1, datFile);
        if (freeChunkBuff)
            free(objEncd.chunk);
...
dllPort bool OBJDAT_FILE::IsFlatRide()
{
