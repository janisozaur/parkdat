...
int dllPort SV6_FILE::GenNewPark(HMODULE module)
{
    ...
    ReadResource(module, "GEN_TEMPLATE", IDR__GEN_TEMPLATE__SV6_HEADER, &h, sz(SV6_HEADER));

    // avail items

    ReadResource(module, "GEN_TEMPLATE", IDR__GEN_TEMPLATE__OBJ_DAT_TABLE, &ai, sz(SV6_OBJDAT_HEADER_BLOCK));
    ...
    ReadResource(module, "GEN_TEMPLATE", IDR__GEN_TEMPLATE__PARK_MAP, &map[0], sz(SV6_PARKMAPDATABLOCK));
    ...
    ReadResource(module, "GEN_TEMPLATE", IDR__GEN_TEMPLATE__PARK_DATA, &pd.parkNameOffset,
                 sz(SV6_PARKDATA) - sz(pd.spriteStats) - sz(pd.sprites));
...
#define setAll(a) memset(&a, be, sz(a))
...
void gnp_SetSpriteStats(SPRITESTATS & ss)
{
    ...
    ss.statsA.firstVehicle = ss.statsA.firstPerson = ss.statsA.firstFloat = ss.statsA.firstJunk =
        ss.statsA.firstOversizedRideCar                                   = we;

    ss.statsA.nextFreeSprite = ss.statsA.vehiCount = ss.statsA.peopleCount = ss.statsA.floatingCount = ss.statsA.junkCount =
...
int dllPort SV6_FILE::ChangeMoney(long newVal)
{
...
dllPort int SV6_FILE::ClearSprites()
{
    ...
    long i, next = 1, fore = RCTX_WORD_EMPTY;
