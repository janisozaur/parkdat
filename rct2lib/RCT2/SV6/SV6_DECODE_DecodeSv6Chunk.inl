DWORD DecodeSv6Chunk(SV6_CHUNKHEADER * dcodChunkHeader, BYTE * dcodChunkData, DWORD dcodChunkMaxSize,
                     SV6_CHUNKHEADER * encdChunkHeader, BYTE * encdChunkData, int i)
{
#define TMPBUF_MAXSIZE 4000000

    long outI, m, inI;
    int  rotCount, encdChunkSize, decodeType;

    char *chunkp, **chunkpp = &chunkp, soffset;
    BYTE  slength, tmpI, leadbyte, dataByte, *tmpbuf, rleLength;
    SV6_CHUNKHEADER
    tmpChunkHeader;

    // debug
    long rle_numRepeatBlocks, rle_numStreamBlocks, decode_completeLength, rle_string_totalReferencedBytes,
        rle_string_totalDirectBytes, rle_string_totalReferences, slength_max, slength_min, rleLength_min, rleLength_max;
...
DoDecodeChunk:
{
    ...
    mm(dcodChunkHeader, &ech, sz(SV6_CHUNKHEADER));
    ...
    case ET_RLE:
        ...
        dcodChunkHeader->chunkSize_Encoded = outI;
    ...
    case ET_RLE_STRING:
        ...
        mm(&tmpChunkHeader, &ech, sz(SV6_CHUNKHEADER));
        tmpbuf = (BYTE *)malloc(TMPBUF_MAXSIZE);

        tmpI = DecodeSv6Chunk(&tmpChunkHeader, tmpbuf, TMPBUF_MAXSIZE, encdChunkHeader, encdChunkData, 0);
...
//#define ASM_RLE_STRING
        ...
        break;
    case ET_ROTATE_1357:
        for (inI = 0, outI = 0, rotCount = 1; ((inI < ecSz) && (outI < dcodChunkMaxSize));
        ...
        dcodChunkHeader->chunkSize_Encoded = ech.chunkSize_Encoded;

        break;

    case ET_LITERAL:
    default:
        mm(dcodChunkData, encdChunkData, __min(ech.chunkSize_Encoded, dcodChunkMaxSize));
        dcodChunkHeader->chunkSize_Encoded = ech.chunkSize_Encoded;
