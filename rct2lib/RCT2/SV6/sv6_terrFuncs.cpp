...
int SV6_FILE::SetMapHeight(int * init, int done, int x, int y, SV6_PARKMAPPTRARRAY * lpMapPtrs, LPSCANLINE scanline,
                           WORD parksize)
{
    short C;
    short N;
...
int SV6_FILE::GetMapHeight(int * init, int done, int x, int y, SV6_PARKMAPPTRARRAY * lpMapPtrs, LPSCANLINE scanline,
                           WORD parksize)
{
...
int SV6_FILE::SaveTerrainBMP()
{
    ...
    */
    ...
    if (!GetSaveFileName_Quick(sz(file) - 1, ~file, OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_LONGNAMES | OFN_OVERWRITEPROMPT,
                               "Windows Bitmap(*.bmp)\0*.bmp\0\0", "BMP", "Save Bitmap Terrain As"))
    {
        return (false);
    }
    ...
    bmpFilehead.bmfh.bfSize = sizeof(BMPFILEHEAD_256) + (bmpFilehead.bmih.bmiHeader.biSizeImage = EvenUp(parksize) * parksize);
...
#undef palcolor
    ...
    ExitFor:
        ...
        if (init)
        {
            if ((hfile = _sopen(~file, _O_BINARY | _O_CREAT | _O_SEQUENTIAL | _O_RDWR, _SH_DENYNO, _S_IREAD | _S_IWRITE)) == -1)
                goto ExitIf;
...
int SV6_FILE::LoadTerrainBMP()
{
    ...
    if (!GetSaveFileName_Quick(sz(file) - 1, ~file, OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_LONGNAMES | OFN_OVERWRITEPROMPT,
                               "Windows Bitmap(*.bmp)\0*.bmp\0\0", "BMP", "Load Bitmap Terrain into Park"))
    {
        return (false);
    }

    hfile = (int)CreateFile(~file, GENERIC_READ, 0, &secuAttrb, OPEN_EXISTING,
