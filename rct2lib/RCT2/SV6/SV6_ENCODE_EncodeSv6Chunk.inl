...
int EncodeSv6Chunk(int encodeType, DWORD encdChunk_maxSize, SV6_CHUNKHEADER * encdChunkHeader, BYTE * encdChunkData,
                   DWORD dcodChunk_maxSize, SV6_CHUNKHEADER * dcodChunkHeader, BYTE * dcodChunkData, int i)
{
#define TMPBUF_MAXSIZE 5000000
#define RLE_MAXCOUNT 125

    long dcodI, encdI, m, tmpI, rleI, dcod_refStartI;

    int rotCount, rleLength;

    char *chunkp, **chunkpp = &chunkp, leadbyte, soffset, leadbyte2;
    ...
    SV6_CHUNKHEADER
    tmpChunkHeader, autoDetect_ChunkHeader;

    union {
        BYTE strRef;
        struct
        {
            BYTE slength_minus_1 : 3, soffset_plus_0x20 : 5;
        };
    };
    ...
    struct
    {
        BYTE flush : 1, type : 2, lasttype : 2, step : 1, init : 1, finished : 1;
    } rle;
//	};
...
DoEncodeChunk:
{
    mm(&ech, dcodChunkHeader, sz(SV6_CHUNKHEADER));

    mm(&autoDetect_ChunkHeader, dcodChunkHeader, sz(SV6_CHUNKHEADER));
    ...
    case ET_RLE:
        ...
        DoRleSwitch:
        {
            ...
            if (dcb == oldDcb) // repeat block
                ...
                rle.type = STREAM;
            rle.flush = ((rle.type != rle.lasttype) && (rle.lasttype != INIT)) || (rleLength >= RLE_MAXCOUNT) || rle.finished;
            ...
            case REPEAT:
                ...
                if (rle.flush)
                {
                    encdI    = RleEncodeFlush(rle.lasttype, ebuf, encdI, rleLength, &rleData[0]);
                    rle.init = true;
                }
                if (rle.init)
                {
    ...
    case ET_RLE_STRING:
        ...
        mm(&tmpChunkHeader, dcodChunkHeader, sz(SV6_CHUNKHEADER));
    ...
    DoStringEncode:
    {
        ...
        while ((dcodI < dcodChunk_maxSize) && (encdI < encdChunk_maxSize))
        {
            ...
            soffset        = GetStringEncodeOffset(&dbuf[dcod_refStartI], &dbuf[dcodI], dcod_refStartI, dcodI, &slength);
        ...
        tmpI = EncodeSv6Chunk(ET_RLE, encdChunk_maxSize, encdChunkHeader, encdChunkData, tmpChunkHeader.chunkSize_Encoded,
                              &tmpChunkHeader, tmpbuf, 0);
    ...
    case ET_ROTATE_1357:
        ...
        for (dcodI = 0, encdI = 0, rotCount = 1; ((dcodI < encdChunkSize) && (encdI < dcodChunk_maxSize));
    ...
    case ET_AUTODETECT:
        ...
        tmpI = EncodeSv6Chunk(autoDetect_ChunkHeader.encodeType, encdChunk_maxSize, encdChunkHeader, encdChunkData,
                              dcodChunk_maxSize, dcodChunkHeader, dcodChunkData, i);
    ...
    default:
        ...
        mm(encdChunkData, dcodChunkData, dcodChunk_maxSize);
